...
MapPointNormal::MapPointNormal(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cld, float radius, const Eigen::Vector2d& origin, const bool weight_intensity, const bool raw) : weight_intensity_(weight_intensity)  {
  ...
  lodestar_odom::timing.Document("Surface points",cells.size());
...
void MapPointNormal::Compensate(const Eigen::Affine3d& Tmot){
  lodestar_odom::Compensate(*input_, Tmot);
  std::vector<double> mot_vek;
  lodestar_odom::Affine3dToVectorXYeZ(Tmot, mot_vek);
  Eigen::Affine2d T(Eigen::Affine2d::Identity());
  downsampled_->clear();
  for(size_t i =0 ; i < cells.size() ; i++){
    double rel_timestamp = GetCellRelTimeStamp(i);
...
double MapPointNormal::GetCellRelTimeStamp(const size_t index){
  const double x = cells[index].u_(0);
  const double y = cells[index].u_(1);
  return lodestar_odom::GetRelTimeStamp(x, y);
}
